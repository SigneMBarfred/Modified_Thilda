...
if i==1
  ...
  elseif i==2
      kAT(i,:)  = -rVa*(-al(6) + mdr(1)- mprm(1)-0.5*0.75*ch4em(k)*1e-3*mgt/sy);        
  elseif i==3
      kAT(i,:)  = -rVa*(-al(7) + AT(3,1)*1/(rVa*150*sy)- 0.007*mprm(1)...     % N2O lifetime 150 yrs; 
          -0.007*0.5*0.75*ch4em(k)*1e-3*mgt/sy);     
  elseif i==4
      kAT(i,:)  =-rVa*(asLL(4) + asHL(4) - al(5) - mdr(1)- mprc(1)- (1-MHC(9))*MHC(8)*GS*mgt/sy ...
          +Wcarb + 2*Wsil -Worg - Volo-0.5*(co2emfos(k)+co2emland(k))*1e-3*mgt/sy );                        
   elseif i==5 
     kAT(i,:)  = -rVa*(asLL(5) + asHL(5)+1.391*mprc(4)...
        ...
        +0.5*(1.391*(co2emfos(k)+co2seq1(k))+1.1*(co2emland(k)+co2seq2(k)))*1e-3*mgt/sy);
    end 
end 
%---------
