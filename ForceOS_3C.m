function [daint,MHC,MHA,MHM,LWR,co2emfos,co2emland,ch4em,co2seq1,co2seq2]=ForceOS_3C(h)
...
co2seq1 = interp1(CO2fosem_A2asf,1:h:336)-interp1(CO2fosem_AG,1:h:336);  
co2seq2 = interp1(CO2landem_A2asf, 1:h:336)-interp1(CO2landem_AG,1:h:336);  
  
% Control parameters for setting long wave outgoing radiation

LWR = [211.13 1.93];         %Clim. Sens. = 3C with LWR = [211.83 1.93]

% Control parameters for anthropogenic calculating CO2 release after 2100
...
CO2tot = 10.85;   
CO2totA2 = 962.7; 
geofrac = 0;   %geofrac = 1 for geological storage and 0 for ocean storage 
frac   = 0.9;  
tCO2   = 280;   
tCO2A2   = 292;
tfrac  = 100;  
...
MHC     = [geofrac tCO2 tCO2A2 A B C D fgeo fact];

%--------------------------------------------
% Control parameters for aerosol forcing after 2100
%--------------------------------------------
